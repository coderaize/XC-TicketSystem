<%@ Page Title="" Language="C#" MasterPageFile="~/Ticket.Master" AutoEventWireup="true" CodeBehind="NATickets.aspx.cs" Inherits="TicketSystem.NATickets" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="body" runat="server">
    ...
    <div class="row">
        <div class="col-md-12">
            <section class="panel">
                <header class="panel-heading" style="padding: 13px !important;">
                    <div class="panel-actions">
                        <a href="#" class="fa fa-caret-down"></a>
                    </div>
                    <div class="row">
                        <div class="col-sm-2">
                            <h2 class="panel-title">Opened Tickets</h2>
                        </div>
                        <div class="col-sm-3">
                ...
                <div class="panel-body" style="display: block; padding-left: 20px !important; padding-right: 20px !important; height: 250px; max-height: 500px; overflow-y: auto">
                    <%foreach (System.Data.DataRow dR in OpenDT.Rows) %>
                    <%{ %>
                    <!-- Open Ticket Item -->
                    <div class="pricing-table">
                        <div class="col-md-3 col-sm-6">
                            <div class="plan">
                                <h3>T#&nbsp;&nbsp;<%= dR["ID"].ToString() %> </h3>
                                <%
                                    TicketSystem.OperationUrl OU = new TicketSystem.OperationUrl
                                    {
                                        DateTime = DateTime.Now,
                                        OperationData = dR["ID"].ToString(),
                                        OperationType = "CLOSE-TICKET"
                                    };
                                    string operationUrlString = TicketSystem.Crypto.EncryptURLAES(Newtonsoft.Json.JsonConvert.SerializeObject(OU));
                                %>
                                <a class="btn btn-sm btn-warning" href="/Operation?Token=<%=operationUrlString%>">Close</a>
                                <a class="btn btn-sm btn-info" target="_blank" href="/TicketView?ID=<%=dR["ID"].ToString() %>">Show</a>
                                <ul>
                                    <li>
                                        <div class="subject-image" data-toggle="tooltip" data-placement="top" title="Subject"></div>
                                        &nbsp;&nbsp;
                                        <b data-toggle="tooltip" data-placement="top" title="Subject: <%= dR["Subject"].ToString()%>" style="white-space: nowrap; overflow: hidden; max-width: 85%;"><%= dR["Subject"].ToString() %></b>
                                    </li>
                                    <li>
                                        <div class="domain-image" data-toggle="tooltip" data-placement="top" title="Domain"></div>
                                        &nbsp&nbsp
                                        <b data-toggle="tooltip" data-placement="top" title="Domain: <%= dR["Domain"].ToString() %>" style="white-space: nowrap; overflow: hidden; max-width: 85%;"><%= dR["Domain"].ToString() %></b>
                                    </li>
                                    <li>
                                        <div class="priority-image" data-toggle="tooltip" data-placement="top" title="Priority"></div>
                                        &nbsp;&nbsp;
                                        <b data-toggle="tooltip" data-placement="top" title="Priority: <%= dR["Priority"].ToString() %>"><%= dR["Priority"].ToString() %></b>
                                        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                        <div class="urgency-image" data-toggle="tooltip" data-placement="top" title="Urgency"></div>
                                        &nbsp;&nbsp;
                                        <b data-toggle="tooltip" data-placement="top" title="Urgency: <%= dR["Urgency"].ToString() %>"><%= dR["Urgency"].ToString() %></b>
                                    </li>
                                    <li>
                                        <div class="clock-image" data-toggle="tooltip" data-placement="top" title="Clock"></div>
                                        &nbsp;&nbsp;
                                        <b data-toggle="tooltip" data-placement="top" title="Time: <%= dR["CreationTime"].ToString() %>"><%= dR["CreationTime"].ToString() %></b>
                                    </li>
                                </ul>
                            </div>
                        </div>
        ...
        <div class="col-md-12">
            <section class="panel">
                ...
                <div class="panel-body" style="display: block; padding-left: 20px !important; padding-right: 20px !important; max-height: 800px; overflow-y: auto">
                    ...
                    <table class="table table-bordered table-striped mb-none" id="na-tickets-table">
                        <thead>
                            <tr>
                                <th>Operations</th>
                                <th>Raised By</th>
                        ...
                        <tbody>
                            ...
                            <%foreach (System.Data.DataRow cDR in ClosedDT.Rows) %>
                            <%{ %>
                            <tr>
                                <td>
                                    <a class="btn btn-info btn-md" target="_blank" href="/TicketView?ID=<%=cDR["ID"].ToString() %>">Show</a>
                                </td>
                                <td><%= cDR["UserEmail"].ToString() %></td>
                                <td><%= cDR["ID"].ToString() %></td>
                                <td><%= cDR["Subject"].ToString() %></td>
                                <td><%= cDR["Domain"].ToString() %></td>
                                <td>P:&nbsp;<b><%= cDR["Priority"].ToString()  %></b>
                                    &nbsp;&nbsp;-&nbsp;&nbsp;
                                    U:&nbsp;<b><%=  cDR["Urgency"].ToString()  %></b>
                                </td>
                                <td><%= cDR["CreationTime"].ToString() %></td>
                                <td><%= cDR["ClosedTime"].ToString() %></td>
                                <td><%= cDR["ClosedByUser"].ToString() %></td>
                            </tr>
                            <%} %>
                        </tbody>
                        <tfoot>
...
<asp:Content ID="Content3" ContentPlaceHolderID="script" runat="server">
    ...
    <script>
        $(document).ready(function () {
            $('#naTicketBtn').siblings().removeClass('nav-active');
            $('#naTicketBtn').addClass("nav-active");


            $('#closed-tickets-table tfoot th').each(function () {
                var title = $(this).text();
                if (title != '') {
            ...
            var table = $('#closed-tickets-table').DataTable({ scrollX: true });
